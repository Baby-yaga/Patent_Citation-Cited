# -*- coding: utf-8 -*-
"""
Created on Sat Jul 26 09:23:56 2025
...
for year in tqdm(range(2020,2025)):
    ...
    # # 处理引证专利列
    # citation_data_split = first_citation_data.drop('引证专利', axis=1).join(
    #     first_citation_data['引证专利'].str.split('; ', expand=True).stack().reset_index(level=1, drop=True).rename('引证专利_split')
    # )
    ...
    data_normal_ = data_normal_[['year', '申请日', '申请号', '公开号', '公开日', '引证专利_split']]
    data_normal_.columns = ['年份', '申请日', '申请号', '公开号', '公开日', '引证专利_1st']
        
    data_normal_.to_csv(road + "一阶引证/{}.csv".format(str(year)), index=False, sep = "|")
